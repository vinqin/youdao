...
if [[ "$basic_pronunciation" != "null" ]]; then
  file_actul_path="${mp3_dir}$basic_pronunciation$*${voice}.mp3"
  if [[ -f "$file_actul_path" && -r "$file_actul_path" ]]; then
    play "$file_actul_path" 2> /dev/null
  else
    gender_flag="female"
    if [[ "$voice" == "1" ]]; then
      gender_flag="male"
...
if [[ "$source_speech" == "true" ]]; then
  file_actul_path="${mp3_dir}src.mp3"
  #echo "$file_actul_path"
  if [[ -f "$file_actul_path" && -r "$file_actul_path" ]]; then
    play "$file_actul_path" 2> /dev/null
  else
...
if [[ "$dest_speech" == "true" ]]; then
  file_actul_path="${mp3_dir}target.mp3"
  #echo "$file_actul_path"
  if [[ -f "$file_actul_path" && -r "$file_actul_path" ]]; then
    play "$file_actul_path" 2> /dev/null
  else
